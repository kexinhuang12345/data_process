...
def launch_training_on_all_splits(experiment: str, splits: List, base_model: str, dropout: float, learning_rate: float):
    ...
    with open("buchwald.csv", 'w') as csvfile:
        fieldnames = ['rxn', 'yield']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

        names = set([i[0] for i in splits])
        for name in names:
            ...
            df = df_doyle.iloc[:][['rxn', 'Output']]
            for rxn, output in zip(list(df['rxn']), list(df['Output'])):
                writer.writerow({'rxn': rxn, 'yield':float(output)/100})
    ...
    #     # for rxn, output in zip(list(train_df['rxn']), list(train_df))
